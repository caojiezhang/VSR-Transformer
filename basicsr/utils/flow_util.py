...
def flowfrombytes(content, is_lmdb, shape):
    h = shape[0]
    w = shape[1]
    if is_lmdb:
        flow = np.frombuffer(content, dtype=np.float32).reshape(h, w, 2)
    else:
        flow = np.frombuffer(content[12:], dtype=np.float32).reshape(h, w, 2)
