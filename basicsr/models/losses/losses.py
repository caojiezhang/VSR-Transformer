...
@weighted_loss
def flow_loss(pred, target, eps=1e-12):
    # print('Before', pred.shape, target.shape)
    pred, target = pred.mean(dim=2), target.mean(dim=2)
    # print('After', pred.shape, target.shape)
    diff = torch.abs(pred - target)
    return torch.sum(torch.heaviside(diff - 10, torch.ones(1, dtype=diff.dtype, device=diff.device)) * diff)
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise
                weights. Default: None.
        """
        return self.loss_weight * flow_loss(pred, target, weight, reduction=self.reduction)
