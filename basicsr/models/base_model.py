...
class BaseModel():
    ...
    def print_network(self, net):
        ...
        logger.info(
            f'Network: {net_cls_str}, with parameters [M]: {net_params:,d}')
        logger.info(net_str)
        
        # pdb.set_trace()
        if 'train' in self.opt['datasets']:
            lq_size = self.opt['datasets']['train']['gt_size'] // 4
            num_frame = self.opt['datasets']['train']['num_frame']
            input_dim = (num_frame, 3, lq_size, lq_size)  # set the input dimension
            ...
            input_x = torch.randn(1, num_frame, 3, lq_size, lq_size).cuda()
            flop_dict, _ = flop_count(net, (input_x, ))
            sumflops = 0
            for i in flop_dict: 
                sumflops = sumflops + flop_dict[i] 
            logger.info('{:>16s} : {:<.4f} [G]'.format('FLOPs(fvcore)', sumflops))
    ...
    def resume_training(self, resume_state, reset_periods=False):
        ...
        for i, s in enumerate(resume_schedulers):
            if reset_periods:
                current_state_dict = self.schedulers[i].state_dict()
                s['periods'] = current_state_dict['periods']
                s['restart_weights'] = current_state_dict['restart_weights']
                s['eta_min'] = current_state_dict['eta_min']
                s['cumulative_period'] = current_state_dict['cumulative_period']
